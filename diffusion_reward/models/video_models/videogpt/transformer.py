...
class VideoGPTTransformer(nn.Module):
    ...
    def calc_sos_tokens(self, x, embs):
        if not self.use_vqemb:
            ...
            device = self.args.device
            sos_tokens = sos_tokens.long().to(device)[:, None, :]
        return sos_tokens

    def output(self, x, compute_joint=True):
        embs, indices = self.encode_to_z(x) # from vqgan
        target = indices
        sos_tokens = self.calc_sos_tokens(x, embs) # from video_gpt
    ...
    def sample(self, embs, x_, c, steps, temperature=1.0, top_k=1):
        ...
        sos_tokens = torch.ones(x.shape[0], 1) * self.sos_token
        sos_tokens = sos_tokens.long().to("cuda:6")
