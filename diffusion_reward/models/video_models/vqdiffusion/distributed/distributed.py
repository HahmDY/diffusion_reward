...
def all_gather(data):
    ...
    tensor = torch.ByteTensor(storage).to("cuda:6")

    local_size = torch.IntTensor([tensor.numel()]).to("cuda:6")
    size_list = [torch.IntTensor([1]).to("cuda:6") for _ in range(world_size)]
    ...
    tensor_list = []
    for _ in size_list:
        tensor_list.append(torch.ByteTensor(size=(max_size,)).to("cuda:6"))

    if local_size != max_size:
        padding = torch.ByteTensor(size=(max_size - local_size,)).to("cuda:6")
