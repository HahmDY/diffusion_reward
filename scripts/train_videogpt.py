        ...
        for epoch in range(args.epochs):
            with tqdm(range(len(train_dataset))) as pbar:
                for i, imgs in zip(pbar, train_dataset):
                    ...
                    loss.backward()
                    self.optim.step()
                    self.writer.add_scalar('Loss/Transformer_Loss', np.round(loss.cpu().detach().numpy().item(), 4), epoch*len(train_dataset)+i)
                    pbar.set_postfix(Transformer_Loss=np.round(loss.cpu().detach().numpy().item(), 4))
                    pbar.update(0)

                val_loss = self.eval(val_dataset)
                self.writer.add_scalar('Loss/Transformer_Val_Loss', val_loss, epoch)
