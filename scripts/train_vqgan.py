...
class TrainVQGAN:
    def __init__(self, args):
        ...
        self.writer = SummaryWriter(f"{self.work_dir}/logs")
        ...
        for epoch in range(args.epochs):
            ...
            with tqdm(range(len(train_dataset))) as pbar:
                for i, imgs in zip(pbar, train_dataset):
                    ...
                    if i % 100 == 0: # visualize 4 images each from real and fake and save at /visualization
                        with torch.no_grad():
                    ...
                    self.writer.add_scalar('Loss/VQ_Loss', np.round(vq_loss.cpu().detach().numpy().item(), 5), epoch*steps_per_epoch+i)
                    self.writer.add_scalar('Loss/GAN_Loss', np.round(gan_loss.cpu().detach().numpy().item(), 3), epoch*steps_per_epoch+i)
                ...
                self.writer.add_scalar('Loss/Val_Loss', np.round(val_loss, 5), epoch)
...
def main(cfg):
    TrainVQGAN(cfg)
    # work_dir = './exp_local/codec_models/vqgan/${domain}'
